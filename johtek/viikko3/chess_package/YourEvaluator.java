...
// IDEAS:
// checking possible moves of each piece, + for own, - for opponent
...
public class YourEvaluator extends Evaluator {
  ...
  public double eval(Position p) {
    ...
    boolean wQueenFound = false;
    boolean bQueenFound = false;
    
    int pieces = 0;
    
    // Deciding wether to go endgame or not
    ...
    if ((!wQueenFound && !bQueenFound) || (wQueenFound && bQueenFound && pieces <= 6)) {
      endgame = true;
    }
    
    // Loop doing simplified material balance and scoring the position of each piece based on the piece squares defined
    for (int x = 0; x < p.board.length; ++x) {
      for (int y = 0; y < p.board[x].length; ++y) {
